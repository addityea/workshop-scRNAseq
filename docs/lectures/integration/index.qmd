...
```{mermaid, echo = FALSE, message=FALSE, warning=FALSE, cache = TRUE}
graph LR
  B{Multiple<br/>Datasets?}
  
  B -->X[Same Experiment] --> C["🔗 INTEGRATION"]
  B -->Y[Multiple Experiments] --> D["🧹 BATCH CORRECTION"] & C
  
  C --> C1["Align Biological<br/>Signals"]
  C1 --> C3["Result: Unified<br/>Cell Types"]
  
  D --> D1["Remove Technical<br/>Noise"]
  D1 --> D3["Result: Clean<br/>Expression"]
  
  C3 --> E["Proceed to<br/>Analysis"]
  D3 --> E

  linkStyle default stroke:#333,stroke-width:3px,stroke-opacity:1;
...
- If batch correction is needed, it's done along with the integration of samples
  - Multiple samples integration is always performed
